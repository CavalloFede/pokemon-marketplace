...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (trade.receiverId !== auth.userId) {
      return res.status(403).json({ error: 'Only the receiver can reject this trade' });
    }
    ...
    const [initiatorPokemon, receiverPokemon] = await Promise.all([
      prisma.userPokemon.findMany({
        where: { id: { in: updatedTrade.initiatorPokemonIds } },
        include: { species: true }
      }),
      prisma.userPokemon.findMany({
        where: { id: { in: updatedTrade.receiverPokemonIds } },
        include: { species: true }
      })
    ]);
